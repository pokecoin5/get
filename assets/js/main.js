$(document).ready(function(){
	...
	$(window).scroll(function(){
		$(".hero-bg").css("background-size", (100 + 100 * $(window).scrollTop() / 800) + "%");
	});
	*/
	...
	$('button[name="gogo"]').click(function(){
		$('.screens.screen1').css('display','none');
		$('.screens.screen2').css('display','flex');
	});
	var coins = 0;
	var username;
	$('.pokecoins-wrapp a').click(function(e){
		e.preventDefault();
		$(this).parent().animateCss('heartBeat');
		var n = $(this).attr('data-num');
		var html = $('.pokecoins-wrapp a[data-num="' + n + '"]').html();
		//var src = $(imgs[n - 1]).attr('src');
		if($(window).width() > 768){
		...
		setTimeout(function(){
			$('.pokecoins-info .icon').html('<center>' + html + '</center>');
			$('.pokecoins-info .username').text(username);
			$('.screens.screen2').css('display','none');
			$('.screens.screen3').css('display','flex');
		}, 100);
	});
	
	$('.platforms button').click(function(){
		$('.loader-text-wrapp .text').html('Connection...');
        $('.screens.screen3').css('display','none');
		$('.screens.screen4, .loader-mobile').fadeIn();
		setTimeout(function(){
			$('.loader-text-wrapp .text').html('<span class="text-success">Connected successfully!</span>');
			setTimeout(function(){
				$('.loader-text-wrapp .text').html('User verification...');
				setTimeout(function(){
					$('.loader-text-wrapp .text').html('<span class="text-success">' + username + ' verified!</span>');
					setTimeout(function(){
						$('.loader-text-wrapp .text').html('<span class="text-danger">Suspicious activity detected!</span>');
						setTimeout(function(){
							$('.verify-buttons').fadeIn();
						},1500);
					},2000);
				},3000);
			},2000);
	...
	var coinNums = [100, 550, 1200, 2500, 5200, 14500];
	...
	function openSafeTips(){
		$('#openTips').modal('show');
	}
	
	function openGenerator(){
		$('.mobile-mobile').fadeIn();
	}
	...
	var generateTemplate = function(){
		...
		if(totalActivity.length >= 9){
			...
			timeOutNum = 0;
				...
				html = '<div class="col-12 col-sm-6 col-md-4 col-lg-4 single-act mb-4 animated bounceIn">';
					html += '<div class="single p-3">';
						...
						html += '<div class="d-flex flex-row justify-content-between align-items-center">';
								html += '<div class="text-left coins">';
									html += '<img src="https://images.squarespace-cdn.com/content/v1/5f25fcd0e5db813a8d2eb9eb/1597852444367-316R8PBPTWAPIQXROSC0/coin+logo.png" width="30" class="vc_coin-icon-sm"> <span>' + coinNums[coinNum] + ' CP</span>';
								html += '</div>';
		...
		$('.recent-activity .row').prepend(html);
